...
import { collectFilePeers, cStyleCopyright, gniFile, makeFileNameFromClassName } from '@idlizer/libohos'
...
function createWriter() {
    return createLanguageWriter(Language.CPP, createEmptyReferenceResolver()) as CppLanguageWriter
}
...
abstract class MultiFileVisitor {
    ...
    printUnitTests() {
        this.library.files.forEach(file => {
            collectFilePeers(this.library, file).forEach(clazz => this.printUnitTestFile(clazz))
        })
    }
...
class MultiFileUnittestVisitor extends MultiFileVisitor {
    ...
    override printUnitTest(clazz: PeerClass): void {
        ...
        Array.from(namespaces.keys()).forEach(namespaceName => {
            namespaces.get(namespaceName)?.forEach(method => this.printMethodUnittests(clazz.componentName, method))
        })
    ...
    getAttrFixtures(attr: TestValue): FixtureType[] {
        ...
        return this.getFixtures(attr.type)
    }
    ...
    appendCommonFixtures(type: TypeHelper, result: FixtureType[]): void {
        ...
        if (result.length == 0) {
            ...
            console.log(`WARNING: getFixtures() for '${typeName}' fixtures not found!`)
        }
    }
    ...
    isOptionsMethod(method: PeerMethod): boolean {
        return OPTIONS_REGEXP.test(method.method.name)
    }
    ...
    printTestBody(
        ...
        let baseMethodName = method.peerMethodName
        ...
        method.argConvertors.forEach((cnv, idx) => {
            let type = TypeHelper.fromMethodArg(this.library, this.aceTypes, method, idx)
            this.state.writer.print(`${type.getTypeName()} initValue${argNames[idx]};`)
        })
        ...
        method.argConvertors.forEach((cnv, idx) => {
            let type = TypeHelper.fromMethodArg(this.library, this.aceTypes, method, idx)
            this.state.writer.print(`${type.getTypeName()} inputValue${argNames[idx]} = initValue${argNames[idx]};`)
        })
    ...
    printMethodUnittests(component: string, method: PeerMethod): void {
        let baseTestName = `${method.peerMethodName}Test`
        ...
        let processArg = (name: string, index: number, ignore = false, parent?: TestValue) => {
            if (idl.isCallback(method.argConvertors[index].idlType)) return false
            let type = TypeHelper.fromMethodArg(this.library, this.aceTypes, method, index)
        ...
        if (this.isOptionsMethod(method)) {
            ...
            if (method.argConvertors.length == 1) {
                ...
                if (margs && margs[0].startsWith('^')) {
                    ...
                    parents.push(attr)
                    method.argConvertors.forEach((arg, index) => {
                        skip ||= !processArg(arg.param, index, false, attr)
                    })
                }
            }
        }
    ...
    printUnitTestHeader(name: string, component: string): void {
        let compDesc = this.aceTypes.getComponents().find(it => it.name == component)
        let disabled = compDesc?.disable?.includes(name)
        this.state.writer.print('')
...
function printModifiersHeaderFile(
    ...
    writer.pushNamespace('OHOS::Ace::NG', false)
    ...
    writer.pushNamespace(standalone ? `TestConst::${clazz}` : '')
...
function printTestFixtures(
    ...
    let header = createWriter()
    let module = createWriter()
    let resTable = createWriter()
    let resIds: string[] = []
    let resStrs: string[] = []
                ...
                if (fixture.type == 'Ark_Resource') {
                    ...
                    module.print(
                        `{"ResId:${resName}_ID", CreateResource(${resName}_ID, ` +
                            `Converter::ResourceType::${fixture.resType}), "${value[1]}"},`
                    )
                    module.print(
                        `{"ResName:${resName}_STR", CreateResource(${resName}_STR, ` +
                            `Converter::ResourceType::${fixture.resType}), "${value[1]}"},`
                    )
                    resTable.print(`{${resName}_ID, ${resName}_STR, ${value[0]}},`)
                    resIds.push(`${resName}_ID`)
                    resStrs.push(`${resName}_STR`)
                } else {
                    module.print(
                        `{"${escapeQuote(value[0])}", ` +
                            `Converter::ArkValue<${fixture.type}>(${value[0]}), "${value[1]}"},`
                    )
                }
            })
            module.popIndent()
        ...
        if (fixture.invalidValues) {
            ...
            fixture.invalidValues.forEach((value, index) => {
                if (fixture.type == 'Ark_Resource') {
                    ...
                    module.print(
                        `{"ResId:${resName}_ID", CreateResource(${resName}_ID, ` +
                            `Converter::ResourceType::${fixture.resType})},`
                    )
                    module.print(
                        `{"ResName:${resName}_STR", CreateResource(${resName}_STR, ` +
                            `Converter::ResourceType::${fixture.resType})},`
                    )
                    resTable.print(`{${resName}_ID, ${resName}_STR, ${value}},`)
                    resIds.push(`${resName}_ID`)
                    resStrs.push(`${resName}_STR`)
                } else {
                    module.print(`{"${escapeQuote(value)}", Converter::ArkValue<${fixture.type}>(${value})},`)
                }
            })
            module.popIndent()
            module.print('};')
    ...
    let headerFile = createWriter()
    let moduleFile = createWriter()
    ...
    moduleFile.pushNamespace('OHOS::Ace::NG::Fixtures', false)
    headerFile.pushNamespace('OHOS::Ace::NG::Fixtures', false)
    ...
    moduleFile.popNamespace(false)
    headerFile.popNamespace(false)
    ...
    // Enums
    let headerEnums = createWriter()
    let moduleEnums = createWriter()
    ...
    moduleEnums.pushNamespace('OHOS::Ace::NG::Fixtures', false)
    headerEnums.pushNamespace('OHOS::Ace::NG::Fixtures', false)
    ...
    moduleEnums.popNamespace(false)
    headerEnums.popNamespace(false)
