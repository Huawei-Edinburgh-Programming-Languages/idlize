...
const NON_JSON_TYPES = ['Ark_CustomObject']
...
export class TypeHelper {
    ...
    static fromMethodArg(library: PeerLibrary, aceTypes: AceTypes, method: PeerMethod, index: number): TypeHelper {
        let decl = method.argConvertors[index].nativeType()
    ...
    getAggregateMembers(): [string, TypeHelper][] {
        ...
        if (idl.isInterface(this.decl)) {
            return this.decl.properties.filter(it => !it.isStatic).map(it => [
                it.name,
                new TypeHelper(this.library, this.aceTypes, it.type, it.isOptional),
            ])
        }
    ...
    isNonJsonType() {
        ...
        if (idl.isInterface(this.decl) && this.decl.properties.length == 1) {
        ...
        if (idl.isInterface(this.decl) && isMaterialized(this.decl, this.library)) return true
        if (idl.isInterface(this.decl)) {
