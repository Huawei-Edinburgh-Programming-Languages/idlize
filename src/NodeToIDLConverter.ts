...
import {
    ...
    getExportedDeclarationNameByDecl,
    identName,
    isDefined,
    isNodePublic,
    isPrivate,
...
import {
    ...
    Context,
    TypeParameterMap,
    generateSyntheticFunctionName, NodeToIDLConverterFactory
} from "./IDLVisitor";

const MaxSyntheticTypeLength = 60

export class NodeToIDLConverter {
    private synthetics: idl.IDLEntry[] = []
    ...
    constructor(
        ...
        private readonly predefinedTypeResolver?: ReferenceResolver,
        private readonly namespaces: string[] = []
    ) {
    }

    getSynthetics(): idl.IDLEntry[] {
    ...
    serializeAmbientModuleDeclaration(node: ts.ModuleDeclaration): idl.IDLModule {
        ...
        return idl.createModuleType(
            name,
            [{
                name: idl.IDLExtendedAttributes.VerbatimDts,
                value: `"${escapeAmbientModuleContent(this.sourceFile, node)}"`
            }]
        )
    }
    ...
    serializeInterface(node: ts.InterfaceDeclaration): idl.IDLInterface {
        ...
        return idl.createInterface(
            ...
            [],
            this.pickInterfaceProperties(allMembers, childNameSuggestion),
    ...
    serializeClass(node: ts.ClassDeclaration): idl.IDLInterface {
        ...
        return idl.createInterface(
            ...
            [],
            this.pickInterfaceProperties(node.members, childNameSuggestion).concat(this.pickAccessors(node.members, childNameSuggestion)),
    ...
    private pickAccessors(members: ReadonlyArray<ts.TypeElement | ts.ClassElement>, nameSuggestion: NameSuggestion | undefined): idl.IDLProperty[] {
    ...
    collectTypeParameters(typeParameters: ts.NodeArray<ts.Node> | undefined): string[] | undefined {
    ...
    protected pickInterfaceProperties(members: ReadonlyArray<ts.TypeElement | ts.ClassElement>, nameSuggestion: NameSuggestion): idl.IDLProperty[] {
        const properties = members
    ...
    private serializeHeritage(heritage: ts.HeritageClause): idl.IDLReferenceType[] {
    ...
    protected isCommonMethodUsedAsProperty(member: ts.ClassElement | ts.TypeElement): member is (ts.MethodDeclaration | ts.MethodSignature) {
    ...
    protected propertyName(name: ts.PropertyName): string | undefined {
        return this.deduceFromComputedProperty(name) ?? nameOrNull(name)
    }

    private deduceFromComputedProperty(name: ts.PropertyName): string | undefined {
    ...
    protected isCommonAttributeMethod(method: ts.MethodDeclaration | ts.MethodSignature): boolean {
    ...
    protected isTypeParameterReference(type: ts.TypeNode): type is ts.TypeReferenceNode {
    ...
    addSyntheticType(entry: idl.IDLEntry) {
        ...
        if (!name || !this.seenNames.has(name)) {
            ...
            this.synthetics.push(entry)
        }
    }

    protected serializeType(type: ts.TypeNode | undefined, nameSuggestion?: NameSuggestion, typeArgs?: ts.NodeArray<ts.TypeNode>): idl.IDLType {
    ...
    protected serializeIntersectionType(node: ts.IntersectionTypeNode, nameSuggestion?: NameSuggestion): idl.IDLInterface {
    ...
    protected computeComponentExtendedAttributes(node: ts.ClassDeclaration | ts.InterfaceDeclaration): idl.IDLExtendedAttribute[] | undefined {
        ...
        if (name && ts.isClassDeclaration(node) && isCommonMethodOrSubclass(this.typeChecker, node)) {
            result.push({
                ...
                value: `"${PeerGeneratorConfig.mapComponentName(name)}"`
            })
        }
    ...
    protected computeExtendedAttributes(
    ...
    protected computeExportAttribute(node: ts.Node, attributes: idl.IDLExtendedAttribute[] = []): idl.IDLExtendedAttribute[] {
        return attributes
    }

    protected computeNamespaceAttribute(): idl.IDLExtendedAttribute[] {
        const namespace = this.namespaces.join(',')
        return namespace ? [{name: idl.IDLExtendedAttributes.Namespace, value: namespace}] : []
    }

    protected computeDeprecatedExtendAttributes(node: ts.Node, attributes: idl.IDLExtendedAttribute[] = []): idl.IDLExtendedAttribute[] {
    ...
    protected serializeObjectType(node: ts.TypeLiteralNode, nameSuggestion: NameSuggestion, typeParameters?: ts.NodeArray<ts.TypeParameterDeclaration>): idl.IDLInterface {
        const properties = this.pickInterfaceProperties(node.members, nameSuggestion ?? NameSuggestion.make("UNDEFINED"))
    ...
    protected pickMethods(members: ReadonlyArray<ts.TypeElement | ts.ClassElement>, nameSuggestion: NameSuggestion): idl.IDLMethod[] {
        return members
    ...
    protected computeClassMemberExtendedAttributes(
    ...
    protected pickConstructors(members: ReadonlyArray<ts.TypeElement>, nameSuggestion: NameSuggestion): idl.IDLConstructor[] {
    ...
    protected pickCallables(members: ReadonlyArray<ts.TypeElement>, nameSuggestion: NameSuggestion): idl.IDLCallable[] {
        return members.filter(ts.isCallSignatureDeclaration)
            .map(it => this.serializeCallable(it, nameSuggestion))
    }

    protected synthesizeTypeLiteralName(properties: idl.IDLProperty[]): string {
    ...
    protected serializeFunctionType(signature: ts.SignatureDeclarationBase, nameSuggestion?: NameSuggestion, extendedAttributes?: idl.IDLExtendedAttribute[]): idl.IDLCallback {
    ...
    protected serializeTupleType(node: ts.TupleTypeNode, nameSuggestion?: NameSuggestion, typeParameters?: ts.NodeArray<ts.Node>, withOperator: boolean = false): idl.IDLInterface {
    ...
    protected serializeParameter(parameter: ts.ParameterDeclaration, nameSuggestion?: NameSuggestion): idl.IDLParameter {
    ...
    protected serializeCallable(method: ts.CallSignatureDeclaration, nameSuggestion: NameSuggestion): idl.IDLCallable {
    ...
    serializeStubbed(node: ts.ClassDeclaration | ts.InterfaceDeclaration | ts.TypeAliasDeclaration | ts.FunctionDeclaration): idl.IDLInterface {
...
export class IDLConverterFactory implements NodeToIDLConverterFactory {
    ...
    create(sourceFile: ts.SourceFile,
        ...
        if (this.language === Language.ARKTS) {
            return new ArkTsIDLConvertor(sourceFile, options, typeChecker, predefinedTypeResolver, namespaces)
        }
        return new NodeToIDLConverter(sourceFile, options, typeChecker, predefinedTypeResolver, namespaces)
    }
}

class ArkTsIDLConvertor extends NodeToIDLConverter {
    protected pickInterfaceProperties(members: ReadonlyArray<ts.TypeElement | ts.ClassElement>, nameSuggestion: NameSuggestion): idl.IDLProperty[] {
        // TODO: ArkTS does not support static filed in interfaces
        const properties = members
            .filter(it => (ts.isPropertySignature(it) || ts.isPropertyDeclaration(it) || this.isCommonMethodUsedAsProperty(it)) && !isPrivate(it.modifiers) && !isStatic(it.modifiers))
            .map(it => this.serializeProperty(it, nameSuggestion))
        return mergeSetGetProperties(properties)
    }
