...
export function identName(node: ts.Node | undefined): string | undefined {
    ...
    if (ts.isImportTypeNode(node)) {
        const qualifier = identString(node.qualifier)
        return qualifier == "Callback" ? qualifier : `imported ${qualifier}`
    }
    ...
    throw new Error(`Unknown: ${ts.SyntaxKind[node.kind]} ${ node.getText()}`)
}
