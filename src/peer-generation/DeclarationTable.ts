...
export class DeclarationTable {
    ...
    generateFirstArgDestruct(name: string, target: DeclarationTarget, printer: IndentedPrinter, isPointer: boolean) {
        ...
        if (isUnion) {
            ...
            /*
            printer.pushIndent()
            printer.print(`${isPointerField ? "&" : ""}${name}${access}array[i];`)
            printer.popIndent()
            */
            printer.print(`}`)
        } else if (isTuple) {
