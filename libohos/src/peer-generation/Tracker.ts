...
class TrackerVisitor {
    ...
    printPeerClass(clazz: PeerClass): void {
        ...
        methods.forEach(method => {
            ...
            //let dname = method.method.name
            //if (dname == `set${clazz.componentName}Options`) dname = `${clazz.componentName}Interface`
            //if (dname == 'construct') dname = 'constructor'
            ...
            if (origName.startsWith("set") || origName.startsWith("get")) {
                if (clazz.decl.methods.findIndex(it => it.name == origName) == -1) {
                    ...
                    if (idx != -1) {
                        ...
                        //let dname = idl.getExtAttribute(prop, idl.IDLExtendedAttributes.DtsName) ?? prop.name
                        this.out.print(`${traceColumns(prop)}|${clazz.className}|${method.method.name}|Property| ${this.tracking(funcKey)}`)
                        return
                    }
                }
            }
            //let dname = method.method.name
            //if (dname == 'ctor') dname = 'constructor'
