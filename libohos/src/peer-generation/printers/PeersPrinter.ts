...
class PeerFileVisitor {
    ...
    protected printImports(peer: PeerClass, imports: ImportsCollector): void {
        ...
        if (this.library.language === Language.TS || this.library.language === Language.ARKTS) {
            ...
            for (const [className, methodMap] of generatorConfiguration().hooks.entries()) {
                console.log(`import class: ${className} hooks: ${methodMap.keys()}`)
                for (const [methodName, hook] of methodMap.entries()) {
                    const hookName = hook ? hook.hookName : `hook${className}${capitalize(methodName)}`
