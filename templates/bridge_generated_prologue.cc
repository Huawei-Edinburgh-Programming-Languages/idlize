...
void impl_EmulateClickEvent(int32_t nodeId, uint8_t* thisArray, int32_t thisLength) {
    // TODO: serialized from application
    Deserializer deserializer(thisArray, thisLength);
    Ark_ClickEvent event = deserializer.readClickEvent();
    GetFullImpl()->getEventsAPI()->getCommonMethodEventsReceiver()->onClick0(nodeId, event); 
}
KOALA_INTEROP_V3(EmulateClickEvent, int32_t, uint8_t*, int32_t)
