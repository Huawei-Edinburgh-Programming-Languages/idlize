...
export function determineInheritanceRole(name: string): InheritanceRole {
    if (generatorConfig().paramArray<string>("rootComponents").includes(name)) return InheritanceRole.Root
    if (generatorConfig().paramArray<string>("standaloneComponents").includes(name)) return InheritanceRole.Standalone
