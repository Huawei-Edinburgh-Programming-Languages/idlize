...
export function qualifiedName(decl: idl.IDLNode, languageOrDelimiter: Language|string, pattern: idl.QNPattern, needCapitalize: boolean = false): string {
    ...
    return idl.getQualifiedName(decl, pattern).split(".").map(it => needCapitalize ? capitalize(it) : it).join(delimiter)
}
