...
interface GenerationOptions {
    emitStub: boolean | "only",
}

export function generateOhos(outDir: string, peerLibrary: PeerLibrary, config: PeerGeneratorConfiguration, options: GenerationOptions) {
    ...
    if (options.emitStub !== "only") {
        generateManaged(ohos, peerLibrary, ohosManagedFiles);
    }

    // NATIVE
    /////////////////////////////////////////

    const native = generateNativeOhos(peerLibrary, options.emitStub)
...
function generateManaged(ohos: OhosInstall, peerLibrary: PeerLibrary, ohosManagedFiles: string[]) {
